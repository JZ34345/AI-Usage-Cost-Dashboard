            ...
            VStack {
                ...
                if appData.dataType == .total {
                    ...
                    if appData.viewType == .graph {
                        ...
                        genericGraph(data: appData.totalGraphData, anomaly: appData.anomalyTotal,
                                     ylabel: "Cost ($)", isDelta: false)
                            .frame(maxWidth: .infinity)
                        
                        if appData.anomalySwitch == .on {
                            AnomalySummaryView(anomalies: appData.anomalyTotal)
                        }
                    //Table
                    } else {
                        ...
                        if appData.anomalySwitch == .on {
                            AnomalyDataTable(anomalies: appData.anomalyTotal)
                            AnomalySummaryView(anomalies: appData.anomalyTotal)
                        } else {
