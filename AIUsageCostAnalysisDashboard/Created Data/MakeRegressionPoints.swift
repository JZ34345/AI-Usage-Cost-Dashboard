...
func makeRegressionPoints(from data: [GenericSummary], daysAhead: Int = 30, category: String) -> [GenericSummary] {
    ...
    let cleanPoints = removeOutliers(points)
    guard let regression = computeLinearRegression(points: cleanPoints) else {return []}
    
    //Calculate for last days
    let lastDate = sorted.last?.day ?? Date()
    let lastDays = lastDate.timeIntervalSince(firstDay) / 86400
    //Changes
    ...
    for day in 1...daysAhead {
        ...
        if regression.rSquared < 0.3 {
            ...
            predictedCost = max(0, regression.predict(x: futureDays))
        }
        
        forecastPoints.append(GenericSummary(day: futureDate, category: category, cost: predictedCost))
    }
    
    return forecastPoints
}

func computeLinearRegression(points: [(x: Double, y: Double)]) -> LinearRegression? {
    let n = Double(points.count)
    guard n > 2 else { return nil }
    
    //All components
    let xSum = points.reduce(0.0) {$0 + $1.x}
    let ySum = points.reduce(0.0) {$0 + $1.y}
    ...
    let rSquared = totalVariance == 0 ? 1.0: 1.0 - (residualVariance / totalVariance)
    
    return LinearRegression(slope: slope, intercept: intercept, rSquared: rSquared, totalVariance: totalVariance, points: points)
}
