...
Overview: displays a swappable view of either **total cost of AI usage or a basic WoW delta graph** as an entry point into the data. The two graphs in the view are simple enough to let users get started while allowing them to see a broad scope of the data used.

Aggregation: displays a view of **cost split by category (Total, Model, Node, Query Lines)**. Multiple categories can be compared with each other at once, but only up to two categories. Allows users to see which category is driving the increase in cost.
...
## Chart types
For each view, the data is visualized using a **line chart or multi-line chart**. This type of chart was chosen as it allows for easy comparisons of different categories over a set period of time. It is more practical to compare across periods of time than bar or stacked bar charts.  
- The best alternative was an area graph. However, the line graph was chosen over an area graph due to the volatility, or lack of clear trends, of the data used, and instances where multiple categories overlap the same area of a chart.
...
- DataType switch: Specifically in overview view. **Switches type of data between total usage cost or week-over-week deltas**. The switch is formatted as a segmented control because it has only two options to choose from.
  
- CostType switch: Not included in overview view due to DataTypeSwitch. **Switches the usage cost of the graph between total cost and average cost**. The switch is formatted as a segmented control because it has only two options to choose from.
...
- Date Filter: Included in all views. It is a menu that **changes the time the data in the graph is displaying** to 7, 30, or 90 days, or a custom input. Custom input must include a start and end date, with both dates in the format of "year-month-day".
  
- Multi-Select: Included in Aggregation and WoW views. It is a menu that **changes what category the data is displayed in, or split into**, in the graph. Multi-Select can display up to two different categories in a chart.
...
- DrillDown Node: Only in the DrillDown view. It is a menu that selects which node in a specified cluster to drill into, displaying usage cost along query types in the specified node. This filter is only activated if the Drilldown Cluster selects a specific cluster.
...
Empty states. What does the app show with 0 records? With one anomalous record? With a missing data day?
